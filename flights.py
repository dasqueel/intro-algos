...
get all possible simple paths from START to END

user dijkstra version
...
def build_graph(flights):
     ...
     for flight, origin, destination, departure, arrival, cost in flights:
         hours, minutes = departure.split(':')
         departure = int(hours) * 60 + int(minutes)
         hours, minutes = arrival.split(':')
         ...
         graph[origin][flight] = weight
         graph[flight][destination] = no_weight
         timing[flight] = (departure, arrival)

     return graph, timing
def find_best_flights(flights, origin, destination):
     graph, timing = build_graph(flights)
     heap = [ ((0, 0), (0), [origin]) ]

     while heap:
          weight, arrival, path = heapq.heappop(heap)
          head = path[-1]

     for successor in graph[head]:
           new_path = path + [successor]
           if destination == successor: return new_path[1::2]

           next_arrival = arrival
           if successor in timing:
                successor_departure, successor_arrival = timing[successor]
                if next_arrival or successor_departure: continue
                next_arrival = successor_arrival

           new_weight = tuple(map(sum, zip(weight, graph[head][successor])))
           heapq.heappush(heap, (new_weight, next_arrival, new_path))
